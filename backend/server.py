...
def _promote_opposition_box_touches(payload: dict, fotmob_players: list) -> int:
    """Inject only the validated FotMob field into SofaScore lineup statistics.

    This lets all existing Player/Leader code consume one canonical stat key while
    preserving SofaScore for every other field. Matching is name-based within the
    already-linked match; no other FotMob metric is promoted here.
    """
    by_name = {}
    for player in fotmob_players or []:
        value = (player.get("stats") or {}).get("Opposition Box Touches")
        if value is not None: by_name[_norm_name(player.get("name"))] = value
    promoted = 0
    lineups = payload.get("lineups", {}) or {}
    for side in ("home", "away"):
        for row in ((lineups.get(side, {}) or {}).get("players", []) or []):
            player = row.get("player", {}) or {}; key = _norm_name(player.get("name"))
            if key not in by_name: continue
            stats = row.setdefault("statistics", {})
            stats["touchesInOppBox"] = by_name[key]
            promoted += 1
    return promoted
...
def import_linked(req: LinkedImportRequest):
    ...
    fotmob_source = (req.fotmob_source or "").strip(); promoted = 0
    if fotmob_source:
        ...
        promoted = _promote_opposition_box_touches(payload, players)
        payload["fotmob"] = {
            "match_id": fotmob_id, "source": fotmob_source, "players": players,
            "team_totals": supplement.get("team_totals", {}),
            "validated_fields": ["Opposition Box Touches"], "provider_role": "supplementary",
            "promoted_player_count": promoted,
        }
        path.write_text(json.dumps(payload, ensure_ascii=False))
    return {"ok": True, "event_id": event_id, "sources": {"sofascore": True, "fotmob": bool(fotmob_source), "fotmob_match_id": payload.get("fotmob", {}).get("match_id"), "validated_fields": payload.get("fotmob", {}).get("validated_fields", []), "promoted_player_count": promoted}}
...
def match_sources(event_id: str):
    ...
    return {"event_id": event_id, "sofascore": {"linked": True, "role": "primary"}, "fotmob": {"linked": bool(fotmob), "role": "supplementary", "match_id": fotmob.get("match_id"), "source": fotmob.get("source"), "player_count": len(fotmob.get("players", []) or []), "validated_fields": fotmob.get("validated_fields", []), "promoted_player_count": fotmob.get("promoted_player_count", 0)}}
